...
\chapter{Uncertainty and decision-making}\label{uncertainty}}

\hypertarget{pre-session-readings-5}{%
\section{Pre-session readings}\label{pre-session-readings-5}}

\emph{Edlin, R., McCabe, C., Hulme, C., Hall, P., \& Wright, J. (2015). Cost effectiveness modelling for health technology assessment: a practical course. Springer.} Chapters 4, 6 and 7.

\emph{Briggs, A., Sculpher, M., \& Claxton, K. (2006). Decision modelling for health economic evaluation. Oxford University Press.}. Chapters 4.1, 4.2 and 4.3.
...
\hypertarget{sources-of-uncertainty-in-cost-effectiveness-models}{%
\section{Sources of uncertainty in cost-effectiveness models}\label{sources-of-uncertainty-in-cost-effectiveness-models}}
...
\hypertarget{structural-uncertainty}{%
\subsection{Structural uncertainty}\label{structural-uncertainty}}
...
\hypertarget{variability-heterogeneity-and-uncertainty}{%
\section{Variability, heterogeneity and uncertainty}\label{variability-heterogeneity-and-uncertainty}}

We must distinguish between \emph{variability} (the differences that occur between patients by chance) and \emph{heterogeneity} (differences that occur between patients that can be explained) from decision \emph{uncertainty} -- the fundamental quantity that we wish to capture from our decision models.
...
\hypertarget{how-to-deal-with-uncertainty}{%
\section{How to deal with uncertainty?}\label{how-to-deal-with-uncertainty}}

Different methods have been developed to address uncertainty in CEA. These methods fall under the umbrella term \emph{sensitivity analysis}. Sensitivity analysis is the process of varying model input values and recording the impact of those changes on the model outputs. The next table shows the different ways to handle uncertainty in CEA.

\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.58}}
  >{\centering\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.42}}@{}}
\toprule
...
\hypertarget{one-way-deterministic-sensitivity-analysis}{%
\subsection{One-way (deterministic) sensitivity analysis}\label{one-way-deterministic-sensitivity-analysis}}
...
{\centering \includegraphics[width=42.51in]{images/tornado} 

}

\caption{Tornado diagram}\label{fig:fig5}
\end{figure}

\hypertarget{probabilistic-sensitivity-analysis-psa}{%
\subsection{Probabilistic Sensitivity Analysis (PSA)}\label{probabilistic-sensitivity-analysis-psa}}
...
\hypertarget{distributions-for-probability-parameters}{%
\subsubsection{Distributions for probability parameters}\label{distributions-for-probability-parameters}}
...
\CommentTok{\#create custom plot of Beta distribution}
\FunctionTok{plot}\NormalTok{(p, }\FunctionTok{dbeta}\NormalTok{(p, }\DecValTok{4}\NormalTok{, }\DecValTok{16}\NormalTok{), }\AttributeTok{ylab=}\StringTok{\textquotesingle{}density\textquotesingle{}}\NormalTok{,}
...
\includegraphics{Advanced-HTA_files/figure-latex/unnamed-chunk-7-1.pdf}
...
\hypertarget{distributions-for-relative-risks-or-odd-ratios-parameters}{%
\subsubsection{Distributions for relative risks (or odd ratios) parameters}\label{distributions-for-relative-risks-or-odd-ratios-parameters}}
...
\begin{Highlighting}[]
\CommentTok{\#define range}
\NormalTok{x }\OtherTok{=} \FunctionTok{seq}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{, }\AttributeTok{length=}\DecValTok{100}\NormalTok{)}

\CommentTok{\#create custom plot of Beta distribution}
\FunctionTok{plot}\NormalTok{(x, }\FunctionTok{dnorm}\NormalTok{(x, }\SpecialCharTok{{-}}\NormalTok{.}\DecValTok{675}\NormalTok{, .}\DecValTok{17}\NormalTok{), }\AttributeTok{ylab=}\StringTok{\textquotesingle{}density\textquotesingle{}}\NormalTok{,}
     \AttributeTok{type =}\StringTok{\textquotesingle{}l\textquotesingle{}}\NormalTok{, }\AttributeTok{col=}\StringTok{\textquotesingle{}red\textquotesingle{}}\NormalTok{, }\AttributeTok{main=}\StringTok{\textquotesingle{}Normal Distribution\textquotesingle{}}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{v=}\SpecialCharTok{{-}}\NormalTok{.}\DecValTok{675}\NormalTok{, }\AttributeTok{col=}\StringTok{"blue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Advanced-HTA_files/figure-latex/unnamed-chunk-8-1.pdf}

\hypertarget{distributions-for-probabilites-with-more-htan-2-categories.}{%
\subsubsection{Distributions for probabilites with more htan 2 categories.}\label{distributions-for-probabilites-with-more-htan-2-categories.}}
...
\hypertarget{distributions-for-costs}{%
\subsubsection{Distributions for costs}\label{distributions-for-costs}}
...
\hypertarget{distributions-for-utilities}{%
\subsubsection{Distributions for utilities}\label{distributions-for-utilities}}
...
\hypertarget{results}{%
\chapter{Presenting results from probabilistic models}\label{results}}
...
\hypertarget{two-treatment-alternatives}{%
\section{Two treatment alternatives}\label{two-treatment-alternatives}}
...
\tightlist
\item
  What is the interpretation of this plane in terms of being cost-effective or not?
\end{enumerate}
...
{\centering \includegraphics[width=55.25in]{images/simulacionmc} 

}

\caption{Estimated joint cost-effectiveness density for a model plotted on the cost-effectiveness plane.}\label{fig:fig6}
\end{figure}

The presentation of the simulated joint density in Figure \ref{fig:fig6} previous requires that the probabilistic output of our probabilistic models is recorded. As this is a straight forward repetitive process, it can be easily achieved in Excel using simple programs known as \texttt{macros}.

\hypertarget{quantifying-uncertainty}{%
\subsection{Quantifying uncertainty}\label{quantifying-uncertainty}}

Interval estimates can be obtained for outcomes of interest using the simulation results by using the percentile method. This simply involves taking the \(\alpha/2\) and \((1 − \alpha/2)\) percentiles of the simulation vector as the \((1 − \alpha)100%
\) uncertainty interval for outcomes of interest.

\hypertarget{aceptability-curves}{%
\subsubsection{Aceptability curves}\label{aceptability-curves}}
...
{\centering \includegraphics[width=42.92in]{images/ceac} 

}

\caption{CEAC.}\label{fig:fig7}
\end{figure}

\hypertarget{the-net-benefit-framework}{%
\subsubsection{The net-benefit framework}\label{the-net-benefit-framework}}
...
\hypertarget{multiple-treatment-options}{%
\section{Multiple treatment options}\label{multiple-treatment-options}}
...
\tightlist
\item
  Chapter 9 in \emph{Edlin, R., McCabe, C., Hulme, C., Hall, P., \& Wright, J. (2015). Cost effectiveness modelling for health technology assessment: a practical course. Springer.}
\item
  To the Markov model built in class (i.e.~\texttt{Markov.R}), add the half-cycle correction as seen in the excel file from class (Chapter 9 from Gray et al.)
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{some.stuff }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{(}\AttributeTok{first.matrix=}\NormalTok{M1, }
                   \AttributeTok{second.matrix=}\NormalTok{M2,    }
                   \AttributeTok{my.data.frame=}\NormalTok{babies,    }
                   \AttributeTok{OldDads=}\NormalTok{babies}\SpecialCharTok{$}\NormalTok{dage }\SpecialCharTok{\textgreater{}} \DecValTok{50}\NormalTok{)    }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{some.stuff}\SpecialCharTok{$}\NormalTok{my.vector[}\DecValTok{3}\NormalTok{] }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{some.stuff}\SpecialCharTok{$}\NormalTok{my.data.frame}\SpecialCharTok{$}\NormalTok{gestation[}\DecValTok{41}\SpecialCharTok{:}\DecValTok{45}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
